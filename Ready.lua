...
local GetTime, HasFullControl, InCombatLockdown, ipairs, IsFalling,
    IsFlying, IsInInstance, IsResting, IsStealthed, next, pairs,
    UnitCastingInfo, UnitChannelInfo, UnitIsDeadOrGhost, UnitInVehicle
    = GetTime, HasFullControl, InCombatLockdown, ipairs, IsFalling,
    IsFlying, IsInInstance, IsResting, IsStealthed, next, pairs,
    UnitCastingInfo, UnitChannelInfo, UnitIsDeadOrGhost, UnitInVehicle
...
-- WoW 12.0: AuraUtil.ForEachAura fails on combat auras (secret values).
-- Use C_UnitAuras.GetPlayerAuraBySpellID for targeted lookups instead.
...
function module:OnInitialize()
    ...
    -- PLAYER_UPDATE_RESTING was removed in 9.0; resting state is now checked in PLAYER_FLAGS_CHANGED
    self:RegisterEvent("PLAYER_FLAGS_CHANGED")
    -- UI_ERROR_MESSAGE type 57 = ERR_MUST_BE_STANDING: detect failed summon while on bench/furniture
...
function module:OnEnable()
    self:PetListUpdated()
    self:ScheduleRepeatingTimer("FrequentCheck", 0.5)
    ...
    self:UPDATE_STEALTH()
    self:RegisterEvent("PLAYER_MOUNT_DISPLAY_CHANGED", "CheckFlying")
    ...
    self:RegisterMessage("PetCall-EnableState", "OnAddOnState")
    ...
    hooksecurefunc("SitStandOrDescendStart", function()
        ...
        if not IsFlying() and not IsMounted() then
            ...
            -- When we just sat down, lock PLAYER_STARTED_MOVING for 3 seconds.
            -- Bench/furniture interaction walks the character to the object AFTER
            -- SitStandOrDescendStart fires, so the movement would otherwise reset
            -- sitting=true before the character reaches the bench.
            if self.is_false["sitting"] then
                self._sit_move_lock = GetTime() + 3
            else
                self._sit_move_lock = nil
            end
        end
    end)
...
function module:UpdateChecks(profile)
    ...
    self:EnableCheck("vehicle", true)           -- vehicle events fire reliably; untested edge cases remain
    ...
    if not next(self.is_false) then
        ...
        if wait_timer_finished then
            if self:LastCheck() then
                ...
                self._last_summon_time = GetTime()
                addon:SummonPet()
...
function module:CheckFlying()
    self:SetReady("flying", not IsFlying())
end

function module:FrequentCheck()
    -- Flying state can change without PLAYER_MOUNT_DISPLAY_CHANGED firing
    -- (e.g. taking off from a stationary flying mount)
    self:SetReady("flying", not IsFlying())

    -- CRZ/phasing can silently dismiss the pet with no COMPANION_UPDATE; poll as fallback
...
function module:PLAYER_STARTED_MOVING(event)
    -- Ignore movement briefly after SitStandOrDescendStart: the character may be
    -- walking to a bench, and the movement should not undo the sitting state.
    if self._sit_move_lock and GetTime() < self._sit_move_lock then
        return
    end
    self._last_summon_time = nil
    self:SetReady("sitting", true)
end

function module:UI_ERROR_MESSAGE(event, errorType, message)
    -- type 57 = ERR_MUST_BE_STANDING ("You must be standing to do that")
    -- If this fires within 3s of a summon attempt, the player is on bench/furniture
    if errorType == 57 and self._last_summon_time and GetTime() - self._last_summon_time < 3 then
        self:SetReady("sitting", false)
    end
end

function module:PLAYER_FLAGS_CHANGED(event)
    -- resting state check (replaces removed PLAYER_UPDATE_RESTING event)
    self:SetReady("resting", IsResting())
