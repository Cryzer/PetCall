...
function lib.new(parent, addonname)
	...
	-- WoW 10.0+: Settings API replaces InterfaceOptions_AddCategory
	-- Use BlizOptionsIDMap + Settings.GetCategory (AceConfigDialog v92+ pattern)
	local displayName = frame.name
	local category
	if parent then
		local acd = LibStub and LibStub("AceConfigDialog-3.0", true)
		local idMap = acd and acd.BlizOptionsIDMap
		local parentID = idMap and (idMap[parent] or parent)
		local parentCat = parentID and Settings.GetCategory(parentID)
		if parentCat then
			category = Settings.RegisterCanvasLayoutSubcategory(parentCat, frame, displayName)
		else
			category = Settings.RegisterCanvasLayoutCategory(frame, displayName)
...
-- WoW 10.1+: GetAddOnMetadata moved to C_AddOns namespace
