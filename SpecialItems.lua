...
-- GetNumQuestLogEntries/GetQuestLink/GetQuestLogTitle deprecated in 9.0 → C_QuestLog.*
-- GetSpellInfo deprecated in 10.1 → C_Spell.GetSpellInfo
...
function module:OnInitialize()
    ...
    -- Pre-request item data for any uncached items so names are ready when options panel opens
    for itemName in pairs(SpecialItems) do
        if strsub(itemName, 1, 5) == "item:" then
            local id = tonumber(strsub(itemName, 6))
...
function module:OnItemInfoReceived(event, itemID, success)
    if success and SpecialItems["item:" .. itemID] then
        AceConfigRegistry:NotifyChange(self.name)
    end
end

function module:UpdateQuestList()
    -- WoW 9.0+: C_QuestLog replaces GetNumQuestLogEntries/GetQuestLink
    wipe(self.currentQuests)
    local numEntries = C_QuestLog.GetNumQuestLogEntries()
    for i = 1, numEntries do
        local info = C_QuestLog.GetInfo(i)
        if info and not info.isHeader then
            self.currentQuests[info.questID] = true
        end
    end
...
function module:AskClientForName(item)
    if strsub(item, 1,5) == "item:" then
        -- Pass numeric ID for reliability; RequestLoadItemDataByID triggers async fetch if not cached
        local id = tonumber(strsub(item, 6))
    ...
    elseif strsub(item, 1,6) == "spell:" then
        -- WoW 10.1+: C_Spell.GetSpellInfo replaces GetSpellInfo
        local spellInfo = C_Spell.GetSpellInfo(tonumber(strsub(item,7)))
        return spellInfo and spellInfo.name
    elseif strsub(item, 1,6) == "quest:" then
        -- WoW 9.0+: C_QuestLog replaces quest log index APIs
        local questid = tonumber(strsub(item,7))
