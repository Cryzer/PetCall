...
-- 3D model preview for broker panel
local brokerPreview
...
local function GetBrokerModelPreview()
    if brokerPreview then return brokerPreview end
    local c = CreateFrame("Frame", "PetCallBrokerPreviewContainer", UIParent, "BackdropTemplate")
    c:SetSize(200, 200)
    ...
    c:SetBackdropColor(0, 0, 0, 0.9)
    c:Hide()
    local m = CreateFrame("PlayerModel", nil, c)
    m:SetPoint("TOPLEFT",     c, "TOPLEFT",      4, -4)
    m:SetPoint("BOTTOMRIGHT", c, "BOTTOMRIGHT",  -4, 4)
    m:SetScript("OnUpdate", function(self, elapsed)
        brokerPreviewAngle = (brokerPreviewAngle + BROKER_ROT_SPEED * elapsed) % (2 * math.pi)
        self:SetFacing(brokerPreviewAngle)
    end)
    c.model = m
    brokerPreview = c
    return c
