...
-- Shared model preview: needs file scope so PetSelectList can hide it on scroll
local modelPreview
...
do
    ...
    -- spin in place.  GetActiveBoundingBox() was removed in 12.0 so we cannot auto-detect
    -- this; add offending IDs here to show a static pose instead.
    -- Find an ID in-game: /run print(select(11, C_PetJournal.GetPetInfoByPetID(petGUID)))
    local STATIC_PREVIEW_CREATURES = {
        [181535] = true,  -- Murkastrasza
    }

    local function GetOrCreateModelPreview()
        if modelPreview then return modelPreview end
        -- Backdrop container (plain Frame — mixing Model + BackdropTemplate breaks Model methods)
        local container = CreateFrame("Frame", "PetCallModelPreviewContainer", UIParent, "BackdropTemplate")
        ...
        -- PlayerModel inherits SetCreature/SetDisplayInfo/SetFacing (plain Model lacks them in 12.0)
        ...
        model:SetScript("OnUpdate", function(self, elapsed)
            if not container.isStatic then
                modelPreviewAngle = (modelPreviewAngle + MODEL_ROT_SPEED * elapsed) % (2 * math.pi)
                self:SetFacing(modelPreviewAngle)
            end
        end)
        model:SetScript("OnModelLoaded", function(self)
            container.isStatic = STATIC_PREVIEW_CREATURES[container.currentCreatureID] or false
            self:SetFacing(0)
            modelPreviewAngle = 0
        end)
        container.model = model

        modelPreview = container
        return container
    end
...
do
    ...
    local function scrollFrame_OnMouseWheel(scrollFrame, value)
        if modelPreview then modelPreview:Hide() end
        GameTooltip:Hide()
