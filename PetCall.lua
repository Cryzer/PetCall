...
local _G = _G
_G.PetCall = LibStub("AceAddon-3.0"):NewAddon(addon, addon_name, "AceConsole-3.0", "AceEvent-3.0", "AceTimer-3.0")

local L = LibStub("AceLocale-3.0"):GetLocale("PetCall")
local LibPetJournal = LibStub("LibPetJournal-2.0")

local assert, C_PetJournal, error, format, getmetatable,
    hooksecurefunc, ipairs, InCombatLockdown, IsInInstance, next, pairs,
    setmetatable, strfind, tinsert, type, wipe
    = assert, C_PetJournal, error, format, getmetatable,
    hooksecurefunc, ipairs, InCombatLockdown, IsInInstance, next, pairs,
    setmetatable, strfind, tinsert, type, wipe
-- fastrandom is WoW-specific; fall back to math.random if it was removed
local fastrandom = fastrandom or math.random
...
_G.BINDING_HEADER_PETCALL = addon_name
_G.BINDING_NAME_PETCALL_SUMMON = L["Summon Another Pet"]
_G.BINDING_NAME_PETCALL_DESUMMON = L["Dismiss Pet"]
_G.BINDING_NAME_PETCALL_TOGGLE = L["Toggle Non-Combat Pet"]
_G.BINDING_NAME_PETCALL_CONFIG = L["Open Configuration"]
...
function addon:EnableSummoning(v)
    ...
    if((not oldv) ~= (not v)) then
        ...
        LibStub("AceConfigRegistry-3.0"):NotifyChange("PetCall")

        -- XXX Use :Enable/:Disable instead
        self:SendMessage("PetCall-EnableState", v)
    end
end
...
function addon:BuildSetWeights(input, output)
    ...
    output.total = 0
    local hasFilters = input.filter and next(input.filter)
    ...
    for _, petID in LibPetJournal:IteratePetIDs() do
        ...
        if hasFilters then
            for _, filterOpt in pairs(input.filter) do
                ...
                if filter and filter:Filter(petID, filterOpt) then
                    local weight = filterOpt[2] or 1
                    tinsert(output, weight)
                    output.total = output.total + weight
                    tinsert(output, petID)
                end
            end
        end
