...
-- FillLocalizedClassList removed; use CLASS_SORT_ORDER + LOCALIZED_CLASS_NAMES_MALE
local error, format, GetSubZoneText, GetZoneText, ipairs,
    ...
    = error, format, GetSubZoneText, GetZoneText, ipairs,
    IsInGroup, IsResting, loadstring, pairs, pcall, print, SecureCmdOptionParse,
    setmetatable, tinsert, tonumber, tostring, type, UnitClass, wipe
...
addon:RegisterTrigger("specialLocation", {
    ...
    OnInitialize = function(self)
        ...
        self:RegisterEvent("ZONE_CHANGED_INDOORS")
        self:RegisterEvent("ZONE_CHANGED_NEW_AREA")
        self:RegisterEvent("PLAYER_FLAGS_CHANGED")  -- replaces PLAYER_UPDATE_RESTING (removed in 9.0)
    end
})
...
addon:RegisterTrigger("macro", {
    ...
    OnInitialize = function(self)
        ...
        self:ScheduleRepeatingTimer("CheckUpdate", 0.2)
    end
})
...
addon:RegisterTrigger("specialization", {
    ...
    FillOptions = function(self, parent, settings)
        ...
        -- WoW 10.0+: FillLocalizedClassList removed; use CLASS_SORT_ORDER global
        for _, classFile in ipairs(CLASS_SORT_ORDER) do
            local localizedName = LOCALIZED_CLASS_NAMES_MALE[classFile]
            if localizedName then
                values[classFile] = localizedName
                tinsert(order, classFile)
            end
        end
